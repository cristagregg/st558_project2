...
In addition to summary statistics, this report will also model bicycle users linear regression, random forests, and boosting. The model will help determine anticipated number of users based on readily available data. To achieve this, the response variables are casual, registered, and cnt. The other 13 variables will be the predictors for models developed later in this report. 
...
Our first linear model has an R^2^ of `r lm.fit$results$RMSE`.  

### Linear Fit 2

Using transformations of predictors identified in the first linear fit and dropping the year predictor (in an effort to forecast future usage). 
